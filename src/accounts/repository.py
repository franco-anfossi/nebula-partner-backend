...
from .user_models import Supplier


async def create_supplier(
    db: AsyncSession, auth_id: str, description: str = None, name: str = None
) -> Supplier:
    """Create a new supplier and save it to the database."""
    new_supplier = Supplier(auth_id=auth_id, description=description, name=name)
    db.add(new_supplier)
    await db.commit()
