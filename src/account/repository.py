...
async def get_account_by_user_and_company(db: AsyncSession, user_id: str, company_id: int) -> Account:
    """Retrieve an account by user profile ID and company ID."""
    result = await db.execute(select(Account).where(Account.user_id == user_id, Account.company_id == company_id))
...
async def update_account_status(db: AsyncSession, user_id: str, company_id: int, is_active: bool) -> Account:
